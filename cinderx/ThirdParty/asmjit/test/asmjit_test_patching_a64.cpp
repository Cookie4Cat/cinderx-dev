...
// need patching when their target is resolved later. Conditional, compare, and
// test branches are only relaxed by a64::Builder; raw a64::Assembler patches
// in-range targets and reports kErrorInvalidDisplacement for out-of-range ones.
...
UNIT(a64_tbz_patching_in_range) {
  ...
  // Forward refs now emit a 4-byte branch and patch the displacement in place.
...
// Test that raw Assembler reports out-of-range tbz. a64::Builder performs the
// relaxation pass for CinderX's main JIT path.
UNIT(a64_tbz_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
  ...
  emitNops(as, 8200);
  EXPECT_EQ(as.bind(target), kErrorInvalidDisplacement);
}

// Test that tbnz (inverse) also reports out-of-range in raw Assembler.
UNIT(a64_tbnz_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
...
// Test that raw Assembler reports out-of-range cbz.
UNIT(a64_cbz_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
...
// Test that cbnz (inverse) also reports out-of-range in raw Assembler.
UNIT(a64_cbnz_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
...
// Test that raw Assembler reports out-of-range b.cond.
UNIT(a64_bcond_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
...
// Test that b.ne (inverse) also reports out-of-range in raw Assembler.
UNIT(a64_bcond_ne_patching_out_of_range_fails_without_builder) {
  CodeHolder code;
  a64::Assembler as;
  setupCode(code, as);
