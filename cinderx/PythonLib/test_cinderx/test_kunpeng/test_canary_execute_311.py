...
class CanaryExecute311Test(unittest.TestCase):
    ...
    def test_function_dies_independently_of_its_pinned_artifact(self):
        ...
        # Python can hold it on its own -- and that is all it holds.  The
        # member set is borrowed, so an external pin on the artifact must
        # not keep the function alive: the function dies with its last
        # reference, the death watch drains the logical registries, and the
        # physical machine code stays resident exactly as long as the pin
        # does.  The function is popped out of its own globals first,
        # because the artifact's CodeRuntime holds the globals dict
        # strongly -- left in place, that chain keeps the function alive
        # for a reason that has nothing to do with the ownership model
        # under test.
        env = dict(os.environ)
        env["CINDERX_JIT_MODE"] = "canary"
        env["PYTHONJITAUTO"] = "1000000"
