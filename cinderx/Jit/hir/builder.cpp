...
BorrowedRef<PyTypeObject> resolveMethodOwnerType(
  ...
  const char* qualname_data =
      PyUnicode_AsUTF8AndSize(qualname, &qualname_size);
  ...
  while (!queue.empty()) {
    ...
    for (auto bc_it = bc_block.begin(); bc_it != bc_block.end(); ++bc_it) {
      ...
      switch (opcode) {
        ...
        case LOAD_GLOBAL: {
          if (tryEmitLoadMethodOrAttrSuper311(irfunc.cfg, tc, bc_it, bc_block)) {
            break;
          }
...
bool HIRBuilder::tryEmitLoadMethodWithValues311(
  ...
  if (owner_type == nullptr ||
      owner_type->tp_version_tag != type_version ||
  ...
  tc.emit<LoadConst>(
      owner,
      Type::fromObject(env_->addReference(BorrowedRef<>{owner_type})));
  ...
  tc.emit<LoadConst>(
      func_reg,
      Type::fromObject(env_->addReference(BorrowedRef<PyFunctionObject>{func})));
...
void HIRBuilder::emitLoadGlobal(
  ...
  auto try_fast_path = [&] {
    ...
    for (auto instr : BytecodeInstructionBlock{code_}) {
      if (instr.opcode() == STORE_GLOBAL && instr.oparg() == name_idx) {
        return false;
      }
    }
