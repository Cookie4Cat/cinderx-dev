...
bool Context::finalizeFunc(
...
#if PY_VERSION_HEX < 0x030C0000
  // Arm the death watch HERE, not inside the registry commit below.
  // NOLINTNEXTLINE
  //
  // Arming it allocates a capsule, a tuple, a C function and a weak
  // reference, so it is the last thing in the publication that can
  // collect, run a finalizer and re-enter Python -- and doing it after
  // the validation would put arbitrary Python between the validation and
  // the commit, which is exactly what the validation exists to exclude.
  // A finalizer there could reassign `__code__` (leaving the registry
  // pointing at an artifact for code the function no longer has) or
  // retire the artifact outright (leaving the commit to insert a pointer
  // whose owner has already torn itself down -- a registry row that
  ...
  // The association and the death watch both allocated -- the first
  // writes the artifact into the function's __dict__, the second builds
  // the weak reference -- and an allocation can collect, and a finalizer
  // can run Python, and 3.11 lets `__code__` be reassigned whenever the
  // free-variable counts match.
  // Everything below is the irreversible half of the publication, so the
  // last look at the subject goes here: an artifact that is not this
  // function's code any more must not reach the registry, the entry
  // point, or the scheduler's attachment budget.  Leaving `installed`
  // false runs the same unwind an allocation failure does, which puts
  // back exactly what predated this attempt.
...
Ref<CompiledFunction> Context::makeCompiledFunction(
  ...
  // The artifact was built for key.code, and publication caches it on
  // THAT code object's extra block.  Everything between the compile and
  // here runs Python -- preloading, the eligibility read above, the
  // allocations publication itself makes -- so a `__code__` assignment in
  // between leaves this function holding a different code object, and
  // publishing anyway would cache this artifact where the guarded entry
  // will find it for code it was not compiled from.  The entry point
  // fails closed on the mismatch at call time, so nothing runs wrong; the
  // damage is bookkeeping -- a machine-code entry per call that bails
  // straight back out, and an artifact filed under the wrong code.  Refuse
  // before anything is published.
  if (func != nullptr &&
