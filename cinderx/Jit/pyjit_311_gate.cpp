...
// On 3.11 State::kRunning is reachable only through the execute-mode
// initialization path (MR-04's canary, MR-11's product spelling), so it
// doubles as the execute-mode predicate.
//
// Live state, so it answers "may machine code run right now".  It is the
// wrong question for choosing which PROTOCOL a request follows, because a
// pause makes it false and the execute protocol is exactly what has to
// handle a pause; use configuredExecuteMode() there.
bool executeMode() {
...
//
// A second function over the SAME key (code, globals, builtins) is a
// different matter: it is the fresh function object 3.11 creates for every
// closure, lambda or comprehension, and it attaches to the artifact its
// code already has (MR-11) instead of compiling again.  The single-owner
// policy that held from MR-04 to MR-10 was a placeholder for the death
// notifications MR-05 delivered: membership is kept honest by the
// function death watch, a member's uncompile retires the artifact for
// every member (as on 3.12+), a member's __code__ swap leaves the others
// untouched, and a member's death removes only that member.
...
extern "C" void* Ci_JitShell311_InstalledArtifact(PyFunctionObject* func) {
  ...
  // Fail closed unless the JIT is usable right now.  disable() deopts the
  // registry and then pauses, but a publication already in flight can
  // finish afterwards -- its association allocates, an allocation can
  // collect, and a finalizer is exactly where a disable() comes from --
  // and it would install this entry point behind disable()'s back.  The
  // newer decision has to win, and this is the one predicate every door
  // consults.  Nothing is torn down: enable() makes the same published
  // artifact serve again.
  if (!jit::isJitUsable()) {
    return nullptr;
  }
...
// The execute-mode arm of the compile entry point: the code object's one
// automatic attempt.  Refusals report their registered reason so observe
// accounting stays closed; the predicate is the one the compile choke
// point uses, so a refusal is reported with its registered reason instead
// of surfacing later as an opaque compile failure.
// Is this refusal a property of the CODE OBJECT?
//
// Only those are verdicts.  The scheduler records a verdict permanently,
// on the code object, where every door reads it -- so a refusal that
// actually describes the FUNCTION would take the code object down with
// it.  The concrete case is a namespace twin: a second function over the
// same code but with foreign globals is refused because the artifact
// belongs to the first one's namespace, and recording that as the code's
// verdict stops legitimate same-namespace instances from ever attaching
// to the artifact that does exist for them.
//
// An allowlist, not a denylist: a refusal reason added later defaults to
// "not a verdict", which costs a re-attempt at worst, where the opposite
...
const char* requestExecute311(
  ...
  // A code object that already carries an artifact this function could
  // share is a fresh ATTACHMENT, not a compile, and the per-code budget
  // is what governs how many of those happen automatically.  Reaching the
  // generic compile entry point here would find the existing artifact and
  // finalize onto it without consulting that budget at all -- with a
  // budget of zero, one instance would still attach and run machine code.
  // The scheduled path therefore goes through the same door the
  // frame-entry attachment uses; force_compile() keeps its own way in,
  // which is what makes it explicit.
...
extern "C" const char* Ci_JitShell311_RequestCompile(
  ...
  try {
    ...
    if (configuredExecuteMode()) {
      // The scheduler counted a specific code object and marked that code
      // object's single attempt as spent.  Everything between the count
      // and here allocates -- registering the outer function arms a death
      // watch, eligibility reads Python objects -- so `__code__` may have
      // been reassigned in between, and 3.11 permits that whenever the
      // free-variable counts match.  The attempt belongs to the code that
      // earned it; a function pointing somewhere else is not this
      // attempt's subject and nothing is recorded against either code.
      if (expected_code != nullptr && func->func_code != expected_code) {
        return CI_JIT_RESULT_311_DEFERRED;
      }
      // Ask before anything is spent.  Everything the scheduler did to get
      // here allocates -- registering the outer function arms a death
      // watch, which builds a weak reference and its callback -- and an
      // allocation can collect, and a finalizer can reach the control
      // plane.  A disable() landing there arrives here as a capability
      // refusal that says nothing about this code object.
      if (Ci_JitShell311_DispatchDeferred()) {
        return CI_JIT_RESULT_311_DEFERRED;
      }
      // Execute: compile, install and let the next call execute machine
      // code.  A verdict other than "installed" that describes the code
      // object itself disables its automatic compilation for good: the
      // scheduler gives a code object one attempt, and the verdict lives
      // on the code object so every door (the frame-entry dispatch, fresh
      // attachment) sees it.  Explicit force_compile() stays available.
      //
      // The code object is captured BEFORE the attempt: compilation runs
      // arbitrary Python (preloading, allocation, finalizers), and a
      // __code__ swap in there would otherwise land this code object's
      // verdict on whatever code the function holds afterwards -- 3.11 has
      // no function watcher to notice the swap.
      // The pinned code, not a fresh read of the function.  Re-reading
      // here would make every check below self-consistent about whatever
      // the function holds NOW: a `__code__` assignment landing between
      // the check above and this line would be compiled, published and
      // recorded as though it were the code that earned the threshold,
      // and every later comparison would agree with it because they all
      // read the same moved target.  The subject of an attempt is fixed
      ...
      if (std::strcmp(verdict, CI_JIT_RESULT_311_INSTALLED) != 0) {
        // Only a refusal that describes the CODE OBJECT is a verdict.
        // The attempt runs arbitrary Python -- preloading, allocation,
        // finalizers -- and a finalizer reaching cinderjit.disable() turns
        // the answer into the capability refusal, which says nothing about
        // this code.  Recording that would make a state the next enable()
        // undoes permanent, so the attempt is reported as withheld and
        // stays available.  This is the one site that writes the verdict,
        // so the whole attempt is covered wherever the decision lands.
        if (Ci_JitShell311_DispatchDeferred()) {
          return CI_JIT_RESULT_311_DEFERRED;
        }
...
extern "C" int Ci_JitShell311_DispatchDeferred(void) {
  ...
  if (jit::autoJitImportDepth() > 0 || jit::autoJitSetupDepth() > 0) {
    // Inside an import or a setup scope.  Everything running here runs
    // once, at a moment that says nothing about whether it is hot: the
    // import machinery's helpers cross any low threshold simply because
    // they run before anything else does.  Suppression belongs with
    // tracing and pause rather than with the refusals -- counting
    // continues, the attempt is not spent, and the first frame after the
    // scope closes dispatches.
    return 1;
  }
  PyThreadState* tstate = PyThreadState_Get();
  if (tstate == nullptr || tstate->c_tracefunc != nullptr ||
      tstate->c_profilefunc != nullptr) {
    // Under a trace or profile function the guarded entry falls back on
    // every call, so a publication made here is reported as a failed
    // attempt -- and the one-attempt rule would make that permanent.
    return 1;
  }
  return 0;
}
...
// How far up the caller chain the outer function is looked for, and how
// deeply a candidate's constants are searched.  A nested function is
// normally called by its outer function or by something the outer called
// (a sort key, a callback) a few frames down; three levels of nesting
// cover a lambda inside a comprehension inside a function.
//
...
int attachFreshImpl311(PyFunctionObject* raw_func) {
  ...
  if (tstate == nullptr || tstate->c_tracefunc != nullptr ||
      tstate->c_profilefunc != nullptr) {
    // Publishing under tracing would install an entry that immediately
    // falls back; wait for a call without it.
    return 0;
  }
  if (jit::autoJitImportDepth() > 0 || jit::autoJitSetupDepth() > 0) {
    // Fresh attachment is a scheduling door too, and it runs BEFORE the
    // threshold path -- an already-dispatched code object reaches it on
    // every later frame, so a suppression that only covered the first
    // dispatch would let an instance created inside an import or a setup
    // scope attach and start running machine code there.  Same transient
    // treatment: nothing is spent, and the next call outside the scope
    // attaches.
    return 0;
  }
