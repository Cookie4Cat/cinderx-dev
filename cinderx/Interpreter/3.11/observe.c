...
//
// Counters live in a private table keyed by code address rather than in
// co_extra, because co_extra costs more than storage: code deallocation
// runs every registered free function for every slot a code object
// carries, so storing there multiplies how often other tenants' callbacks
// fire -- including ctypes callbacks into Python during interpreter
// shutdown (test_code registers exactly that).
//
// The table does still need co_extra for one thing.  An address-keyed
// table has to know when its keys die, and 3.11 has no code watcher; a
// weak reference would answer that, but it would answer it to everyone,
// because weakref.getweakrefs() hands a referent's whole weakref list to
// any caller and observation must not be visible to the program being
// observed.  So watching a code object creates the block whose free
// function reports its death, and nothing else is stored in it here.
// That is what forces the walk past foreign slots below ours -- which is
// why every mode above `off` refuses to configure unless CinderX holds
// slot zero (see Ci_Observe311_Configure).
...
int Ci_Observe311_Configure(void) {
  ...
  // Every mode above `off` watches code objects, and watching one means
  // storing into its co_extra so the death notice fires.  CPython 3.11's
  // code_dealloc then walks slots 0..ce_size calling every registered
  // freefunc -- including foreign slots BELOW ours, for code objects that
  // never populated them.  A foreign freefunc that outlives nothing in
  // particular is merely called for free; a mortal one (test.test_code
  // registers a ctypes callback at import) can already be dead when the
  // last code objects are torn down at shutdown, and the walk then jumps
  // into a freed trampoline.
  //
  // Capping what we allocate protects the slots above ours.  Nothing can
  // protect the ones below, so the only honest answer is to refuse the
  // configuration.  The execute path already refused it; observation and
  // shadow reach the same co_extra and must refuse it on the same terms.
  // A normal startup claims slot 0 long before user code runs, so this
  // costs the product path nothing.
...
// A code object this table has an entry for is being destroyed.
//
// This is how the observer learns about death, and it is the reason the
// table holds no reference of any kind.  A weak reference would answer the
// same question, but it would answer it to everyone: a weak reference
// lives on its referent's weakref list, which `weakref.getweakrefs()` and
// `weakref.getweakrefcount()` return verbatim, so observing a code object
// would be visible to the program being observed -- with a callback or
// without one.  The code-extra free function is CPython 3.11's only other
// death signal (there is no code watcher), it is already the one this
// runtime uses, and it is invisible from Python.
//
// Called from code_dealloc.  The argument is a key, never dereferenced,
// and nothing here allocates, grows or frees: the table shape cannot
// change under a frame that is mid-lookup.
...
// runs from code_dealloc for every index the code object has a block for
// -- so the block has to exist before the entry does.  Creating it is the
// whole cost of watching a code object, and it replaces the weak
// reference that used to be created here.  Unlike that weak reference it
// allocates from the raw allocator rather than the GC's, so the table
// path can no longer trigger a collection: nothing can die, or be
// created, while a frame is mid-lookup.
static int observe_arm_death_notice(PyCodeObject* code) {
  return codeExtra(code) != NULL ? 0 : -1;
}

// Rehash the table, dropping entries whose code died.
//
// Capacity is chosen from what is still WATCHED, not from what is keyed.
// A slot keeps its key after its code object dies -- cutting the key would
// cut every probe chain running through it -- so a workload that observes
// many short-lived code objects fills the table with tombstones and would
// trip the load factor with almost nothing alive in it.  Doubling there
// buys memory to hold entries that are already garbage, and the footprint
// would ratchet up an octave per churn cycle while the live population
// stayed flat.  Compacting at the same capacity reclaims exactly those
// slots.  The threshold to actually grow is half, not the load factor
// that triggered the rehash: leaving after a compaction at 74% would
// make the next handful of insertions rehash again.
...
static void observe_on_frame_locked(
    ...
    PyCodeObject* code,
    struct _PyInterpreterFrame* frame) {
  // Which code object is running is the FRAME's answer, not the
  // function's.  A frame takes its own strong reference to the code it
  // was built for, and `function.__code__` may be reassigned afterwards
  // -- 3.11 only requires the free-variable counts to match -- while a
  // suspended generator keeps resuming the frame it already has.
  //
  // Every resume of such a frame would otherwise be counted against the
  // code the function holds NOW, which is not the code being executed.
  // That code reaches the threshold, and spends its single automatic
  // attempt, without ever having run a frame: either it is compiled from
  // no execution history at all -- the premature-compilation state the
  // maturity policy exists to avoid -- or its refusal is recorded as a
  // verdict nothing it did earned.
  //
  // Scheduling is skipped outright rather than redirected to the frame's
  // code.  Publication needs a function whose code is the one compiled,
  // and this function's is not; counting a code object this door can
  // never publish would only build up a threshold that has to be
  // discarded.  The frame runs interpreted, exactly as before.
