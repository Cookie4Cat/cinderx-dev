...
TEST_F(BranchRelaxationTest, RawAssemblerInRangeBranchHasNoNopPadding) {
  asmjit::CodeHolder code;
  code.init(code_allocator_->asmJitEnvironment());
  asmjit::a64::Assembler as(&code);

  asmjit::Label target = as.newLabel();

  EXPECT_EQ(as.cbz(asmjit::a64::x0, target), asmjit::kErrorOk);
  EXPECT_EQ(as.mov(asmjit::a64::x0, 42), asmjit::kErrorOk);
  EXPECT_EQ(as.bind(target), asmjit::kErrorOk);
  EXPECT_EQ(as.ret(asmjit::a64::x30), asmjit::kErrorOk);

  // Raw Assembler forward conditional branches are patched in place. Branch
  // relaxation is a Builder pass, so in-range raw Assembler branches should not
  // keep the old branch+NOP placeholder shape.
  EXPECT_EQ(code.textSection()->bufferSize(), 12u);
  EXPECT_EQ(countNops(code), 0u);
}

TEST_F(BranchRelaxationTest, RawAssemblerOutOfRangeBranchFails) {
  asmjit::CodeHolder code;
  code.init(code_allocator_->asmJitEnvironment());
  asmjit::a64::Assembler as(&code);
  ...
  for (int i = 0; i < 270000; i++) {
    EXPECT_EQ(as.nop(), asmjit::kErrorOk);
  }

  // CinderX's main JIT path uses Builder, which relaxes this form. Raw
  // Assembler does not run the relaxation pass and must report the range error.
  EXPECT_EQ(as.bind(target), asmjit::kErrorInvalidDisplacement);
}
