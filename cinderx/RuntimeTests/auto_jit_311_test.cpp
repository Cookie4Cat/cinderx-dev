...
TEST_F(AutoJit311Test, GeneratorsAreRefusedAutomaticallyButCompileOnRequest) {
  SKIP_311_EXECUTABLE_COMPILE();
...
def gen(n):
  ...
  Ref<PyFunctionObject> gen(compileAndGet(src, "gen"));
  ASSERT_NE(gen, nullptr);
  EXPECT_STREQ(
      Ci_JitShell311_RequestCompile(
          gen, reinterpret_cast<PyCodeObject*>(gen->func_code)),
      "REFUSE_SHAPE_GENERATOR_AUTO_DISABLED");
  EXPECT_TRUE(
      codeAutoJitDisabled311(reinterpret_cast<PyCodeObject*>(gen->func_code)));
  EXPECT_FALSE(isJitCompiled(gen));
  if (jit::getConfig().sync_generator_jit) {
    EXPECT_EQ(jit::compileFunction(gen), jit::Result::OK);
    EXPECT_TRUE(isJitCompiled(gen));
  }
  EXPECT_EQ(PyErr_Occurred(), nullptr);
}
...
TEST_F(AutoJit311Test, OuterRowsAreErasedAcrossACodeSwap) {
  ...
  // The outer-function map holds borrowed function pointers, and the death
  // notification is what keeps them honest.  Unregistration looks the rows
  // up through the function's CURRENT code object, so a function whose
  // __code__ was replaced after it was registered must still have its rows
  // erased -- 3.11 has no function watcher to notice the swap, so nothing
  // re-registers in between.  A surviving row would name freed memory, and
  // the publication path dereferences it (globals, builtins, __dict__).
  runCode(kFactorySource);
  Ref<> factory_obj = getGlobal("factory");
  BorrowedRef<PyFunctionObject> factory{factory_obj.get()};
  ASSERT_NE(factory, nullptr);
...
TEST_F(AutoJit311Test, AFailedCodeExtraAllocationLeavesTheGaugeBalanced) {
  // resident_code_extra_blocks is a residency gauge the lifecycle census
  // reads as an invariant, so it has to be balanced on every path -- and
  // the one path that gives a block back is the one where storing it into
  // co_extra fails.  A gauge that only ever rises would report a leak on
  // the single path that did not leak, and the census would call it a
  // finding forever.
  // Nothing here needs machine code, but every case in this file runs in
  // the canary arm: the green arm does not carry AutoJit311Test, so a
  // case without the gate would run in neither.
  SKIP_311_EXECUTABLE_COMPILE();
  const char* source =
      "def gauge_probe(a, b):\n"
      "    total = a + b\n"
