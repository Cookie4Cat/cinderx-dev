...
TEST_F(
  ...
  // Retiring the artifact releases its runtime references, and a __del__
  // riding one of them can reenter force_compile() for the same function
  // -- rebuilding compilation state after this operation tore it down.
  // The newer decision wins the world; force_uncompile() must not report
  // success over it.  The verdict is therefore computed after every
  // release, the symmetric rule to force_compile()'s post-drain
  // re-verification.
  const char* py_src = R"(
events = []
...
TEST_F(JITLifecycle311Test, ForceUncompileIgnoresForgedForeignAnchor) {
  ...
  // The dictionary key is user-writable Python state; the ownership
  // oracle is the association.  A forged foreign value at the key can be
  // the last reference keeping ANOTHER function's artifact alive, and
  // force_uncompile(f1) detaining-and-releasing it uncompiled f2 as a
  // side effect.  The operation now touches the key only when the value
  // is the claimed artifact itself; anything else stays exactly as the
  // user wrote it, and the neighbour keeps its machine code.
  const char* py_src = R"(
def keeper(a, b):
    total = a - a
    while total < b:
...
TEST_F(
  ...
  // The same-key variant of the reentrant recompile: the function's code
  // never changes, so the successor the reentry publishes lives under the
  // SAME CompilationKey the retirement is about to erase.  A dying
  // generation removes a compiled-codes entry only while the entry is
  // still its own; erased blindly by key, the successor stayed installed,
  // associated and anchored while lookupFunc() answered null -- and,
  // missing from the map the destructor pins, it dodged teardown severing
  // and left its owner pointer dangling past the context.
  const char* py_src = R"(
events = []
...
TEST_F(JITLifecycle311Test, StaleArtifactDeathPreservesSameKeySuccessor) {
  ...
  // The death-path sequence pin: an externally pinned stale artifact dies
  // AFTER a successor was published under the same key, and the successor
  // must survive the death.  Today the stale generation arrives at its
  // destructor already cleared -- force_uncompile() cleared it, and
  // clear() is owner-gated -- so its death touches no map; the identity
  // guard on forgetCompiledFunction()'s compiled-codes erase is the
  // backstop for any future path that reaches death uncleared, extending
  // the only-what-it-still-owns rule the registries already follow.
  const char* py_src = R"(
def perennial(a, b):
    total = a - a
    while total < b:
