...
structure_key 打包、opcode 家族表、loop_score、特化观测、滞回、CodeExtra 原子发布、release/acquire、jitVectorcall、AutoJitPolicy。

## 5 Abstract 摘要

本详细设计将功能设计落到可指导编码的 C++ 实现：(1) 数据结构与编码——`StructureKey` 打包为单字、`CodeExtra` 扩展一个原子发布的 `skey_word`、`opcode→工作维度` 家族表；(2) 单次字节码扫描与 `structure_key` 派生算法；(3) 特化观测与滞回带（**v1 不实现，defer 到 Phase-3，审校 T2.2**）；(4) `CodeExtra` 的 free-threaded 单字 release/acquire 发布与失败回退；(5) `jitVectorcall` 集成与**最小阈值策略** `computeThreshold`（自由函数，对低 ROI 族抬阈值削减 compile storm，T3.1b/T2.1）。设计以单字原子发布消除"值/标志"排序风险，并对每个单元给出正常/异常行为模型、数据流转、内部接口定义与 DFX。
...
| 上游 | 关键约束（本详细设计须满足） |
|---|---|
| 需求 R18 | `structure_key` 是唯一聚合键；`specialization_band` 不进键、不聚合 |
...
**术语对应（审校 T4.2）：** 上游需求/功能设计中的 `structure_key` 指**逻辑聚合身份**（解码后的 `StructureKey` 值）；本详细设计的 `skey_word` 指其**物理容器**（含 valid 位 + payload 的 32 bit 字）。凡缓存、统计、聚合契约中说"structure_key"，一律指解码值，绝不指原始字。数据流：`StructureKey`(值) → `pack()` → payload → `skey_word`(release 写) → acquire 读 → `unpack()` → `StructureKey`(值)。
...
skey_word (uint32_t) 位布局：
  ...
  bits [8..11]  family          # Family 枚举，9 个值（T2.4 去 ScalarCompute），占 4 bit
  bits [6..7]   loop_score      # 0..3
  bit  5        is_suspendable
  bit  4        is_static
  bit  3        high_risk
  bits [0..2]   reserved (=0)
PAYLOAD_MASK = 0x0FFF ; VALID_BIT = 0x8000_0000
```
...
打包不产生异常。解包仅在 `valid` 位为 1 时进行（由缓存层 9 的读路径保证）。`Family` 越界不可能（由派生逻辑枚举闭集保证）。
...
enum class Family : uint8_t {
  ...
  ImportInit, Trivial, Mixed,
  kCount
};
// kCount==9，须与需求 R15 的 family 枚举逐一对应（新增 family 须同步改 R15 与位宽）
static_assert(static_cast<int>(Family::kCount) == 9, "family count must match requirements R15");
...
对 code object 单次遍历 `BytecodeInstructionBlock`，累积 6 工作维度计数、异常子计数、可特化/已特化计数、并由 OSR backedge 求 `loop_score`，最终派生 `StructureKey`。对应功能项 1（R1–R16、R19、R22–R25）。
...
Signature scanCode(BorrowedRef<PyCodeObject> code) {
  ...
  for (auto it = block.begin(); it != block.end(); ++it) {
    ...
    // —— 后向边就地收集（T1.3）——
    if (isBackwardJump(op)) {                       // JUMP_BACKWARD / JUMP_BACKWARD_NO_INTERRUPT（3.14）
      int tgt = instr.getJumpTarget().value();      // BytecodeInstruction::getJumpTarget()
      if (tgt < instr.index().value() && s.backedges.size() < 16)
        s.backedges.push_back({instr.index().value(), tgt});
    }
    s.n_eff++;
  }
...
用扫描中就地收集的后向边端点（10.2.1），将每条边视为区间 `[target_idx, source_idx]`，最大嵌套深度即"任一点被覆盖的区间数上限"，事件法求得后截断到 `0..3`。**无第二次扫描、无堆分配**（边集为栈上 `SmallVector`，上限 16）：
...
uint8_t loopScore(const llvm::SmallVectorImpl<std::pair<int,int>>& edges) {
  ...
  return (uint8_t)std::min(depth, 3);
}
```
...
StructureKey deriveStructureKey(BorrowedRef<PyCodeObject> code) {
  ...
  // 预过滤 ImportInit（R12）：缺 OPTIMIZED|NEWLOCALS 或 <module>
  if (!hasRequiredFlags(code) || isModuleName(code)) {
    k.family = Family::ImportInit; k.loop_score = 0; return k;
  }
  ...
  auto buckets = bucketize(s);                                        // R14（density+floor）
  auto [d1, d2] = topTwo(buckets);                                    // 主导 / 次主导
  if (bucket(buckets,d1) - bucket(buckets,d2) < kDelta
      && isModerate(buckets,d1)) {                                    // R16
    k.family = Family::Mixed; return k;
  }
  k.family = mapDominant(d1, k.loop_score);                           // R15 映射表
  return k;
}
```
...
bool deriveRisk(const Signature& s, BorrowedRef<PyCodeObject> code) {
  ...
  bool synthetic  = isSyntheticFilename(code);
  return async_sm || heavy_dyn || exc_heavy || huge || synthetic;
}
```
...
- 默认常量（`kDelta`、`kExcRatio`、`kHugeCodeLen`、各维 cutoff、floor）集中为可调常量并标注"待采样标定"（需求 Outstanding）。
...
把 `StructureKey` 以**单字 release/acquire** 发布进 `CodeExtra.skey_word`，命中即 O(1)，未命中扫描一次；并发首次良性；缓存不可用回退默认阈值。对应功能项 3（R21、R26、KD8）。

## 12.2 关键算法与流程

```cpp
// 返回 std::optional：nullopt 表示缓存不可用 → 调用方回退默认阈值（KD8(c)）
std::optional<StructureKey> getOrComputeStructureKey(BorrowedRef<PyCodeObject> code) {
  CodeExtra* ex = codeExtra(code);                 // 既有 get-or-create（CriticalSection 保护）
...
首次：未命中 → 扫描 → release 发布。后续：acquire 命中 → 解包返回。
...
`getOrComputeStructureKey` 是单元 13 的唯一入口；内部调用单元 10 的 `deriveStructureKey`。（Phase-3 恢复特化观测时可加 `Signature* out_sig` 回填以复用同一次扫描。）
...
- `CodeExtra` 新增字段（v1 仅 `skey_word`）置于 `jit_builtins` 之后，避免改动 `union calls/next` 与既有偏移敏感代码；`PyMem_Calloc` 已零初始化 → `skey_word=0` 即未分类。（`spec_band` Phase-3 才加。）
...
> **审校决策已回灌：** T3.1(b) 默认策略不再是 no-op，而是对低 ROI 族抬阈值削减 compile storm；T2.1 `AutoJitPolicy` 虚类降为自由函数 `computeThreshold`；T2.2 v1 不读特化观测，`gate_view` 仅含 `structure_key`；T2.3 不新增环境变量，复用 `PYTHONJITAUTO=auto[:N]` 启用。

## 13.1 实现概述

在 `jitVectorcall`（`cinderx/Jit/pyjit.cpp:183`，阈值门 `:197`）注入一次分类 + 阈值计算；提供**最小有用策略**（对确定低 ROI 族抬阈值，其余族走现状），使 v1 即有可测收益。**启用复用 `PYTHONJITAUTO`（不新增 env，T2.3）**：`=auto[:N]` 开分类、`=<N>` 回到现状固定阈值。对应功能项 4（R18、R20、R26、KD2/KD8、T3.1b）。
...
uint64_t calls = countCalls(code);
...
if (!getConfig().auto_classify) {                        // PYTHONJITAUTO=数值 → 分类关，等价现状（T2.3）
  limit = global;
} else if (auto sk = getOrComputeStructureKey(code); !sk.has_value()) {
  limit = global;                                        // 缓存不可用 → 回退（KD8/R26）
} else {
  limit = computeThreshold(*sk, global);                 // 单元 13 最小策略（无 SpecBand，T2.2）
}
...
```cpp
// 对确定低 ROI 的族抬高阈值（晚编译/几乎不编译），削减启动期 compile storm；
// 其余族走现状全局阈值。倍数为可 env 覆盖的保守默认（T3.3）。
uint32_t computeThreshold(const StructureKey& sk, uint32_t global) {
  bool low_roi = sk.family == Family::ImportInit
              || sk.family == Family::Trivial
              || sk.high_risk;                           // synthetic/async-sm/超大码 已并入 high_risk(R8)
  if (low_roi) {
    return saturating_mul(global, kLowRoiThresholdFactor);   // 默认 ×N，env 可覆盖
  }
  return global;
}
...
分类成功且开关开 → `computeThreshold` 给阈值（低 ROI 族抬高，其余=全局）→ 解释/编译二分不变。
...
`computeThreshold` 为自由函数（T2.1，无策略对象/单例）；新增配置项 `config.auto_classify`（bool，由 `PYTHONJITAUTO=auto[:N]` 解析置真，T2.3）；`kLowRoiThresholdFactor` 为低 ROI 族阈值倍数（可 env 覆盖，T3.3）；`kNoAutoJit` 表"AutoJIT 未启用"的哨兵。

### 13.4.2 数据流转

`StructureKey` → `computeThreshold` → `limit` → 与 `calls` 比较。聚合统计（下游）按 `StructureKey` 落库（R18）。**v1 无 `SpecBand`（T2.2）。**

> 术语对应（审校 T4.7）：需求 R18 的 `gate_view` 是**概念名**；v1 中它就等于 `structure_key`（特化观测 defer 后不再有第二分量），不引入 `gate_view` 结构体。

## 13.5 接口设计

### 13.5.1 内部接口设计

`jitVectorcall` 只依赖 `getOrComputeStructureKey` + `computeThreshold`。`computeThreshold` 是唯一阈值决策点；下游升级策略时替换其实现（或在此提升为接口），不触碰分类器与 gate。

### 13.5.2 内部接口定义

```cpp
uint32_t computeThreshold(const StructureKey& sk, uint32_t global);  // 自由函数（T2.1）
...
- **回归基线**：开关关时编译函数集合与现状逐函数 bit-for-bit 一致（CI 守门）；开关开时仅低 ROI 族编译时机后移，可单独验证收益。
- `kLowRoiThresholdFactor` 等常量集中为可 env 覆盖的保守默认，正式值按 T3.3 标定协议（混合语料）确定。
...
| 统计被切碎（band 入键） | 类型隔离 + 聚合契约 + 评审守门（单元 11.5/13.5） | AE8 + 代码评审 |
| 新 opcode 漏归类 | 家族表全 opcode 覆盖单测 | 单元 9.6 单测 |
| 误把"曾单态"当稳定 | band 弱语义 + 限幅 + 滞回 | AE10 |
| 分类器异常拖垮 gate | 纯计算无抛异常路径；失败→回退默认阈值 | AE11 + 故障注入 |

## 14.2 异常处理设计

- **缓存不可用**：`getOrComputeStructureKey` 返回 `nullopt`，gate 回退 `config.compile_after_n_calls`（KD8(c)）。不崩、不读半态。
...
- 首次路径：**单次** O(n) 扫描（n=指令数，工作维度 + 特化 + 后向边一遍完成，T1.3）+ 一次 release 发布。**不再有第二遍 backedge 扫描。**
...
- **验收标准（R21，审校 T4.6 补全）：**
  ...
  - (V3) 等价性：默认策略下，编译函数集合与调用计数与现状**逐函数 bit-for-bit 一致**（CI 回归对比，作为分类/缓存基建零行为变更的硬门）。
...
| SP/flags | `cinderx/Jit/hir/preload.cpp:449` `CI_CO_STATICALLY_COMPILED`；`pyjit.cpp:96` `required_code_flags` |
