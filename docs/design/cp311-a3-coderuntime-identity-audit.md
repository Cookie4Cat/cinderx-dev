...
| `GenDataFooter::code_rt` | suspended-generator borrow | **Was an unprotected borrow.** The footer now holds a strong reference to the owning artifact (`GenDataFooter::artifact`, CPython 3.11), so the runtime and its machine code live at least as long as any generator that can resume into them. Released when the object stops being a JIT generator (`deopt_jit_gen_object_only`); visited by `jitgen_traverse` so generator cycles stay collectable. |
