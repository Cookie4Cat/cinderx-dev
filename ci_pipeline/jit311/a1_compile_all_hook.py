"""Test-only A1 Compile-All hook; installed through staged sitecustomize.
...
def _install_generator_capi_adapter() -> None:
    ...
    applies this adapter; A1-C stages the same policy explicitly so Compile-All
    cannot bypass it.
    """
    if _MODE != "jit":
        return
    ...
    if original is None or getattr(original, "__a1_generator_adapter__", False):
        return
    ...
    adapted.__a1_generator_adapter__ = True
