...
def stdlib_canary_runner(*, judges: list[Judge] | None = None) -> RunnerSpec:
    ...
    #
    # The organic count steps with each milestone: five in
    # the MR-07 era, one more when MR-08 opened the exception family, 333
    # once MR-09's pull-validated attribute caches guard attribute sites
    # across 72 importing modules (import-time class mutation retires
    # receiver-version guards by design), and 340 at the MR-11 base: the
    # M09 review's last rounds (type-receiver caches pull-validated
    # instead of closed) moved the count after the 333 pin was taken, and
    # MR-11 itself leaves it unchanged -- the base build and this branch
    # measure 340 alike in the gate's invocation context.  The count is
    # sensitive to sys.path (the repository root on PYTHONPATH changes the
    # compiled slice and reads 343); the gate runs with the venv only.
    # Verified deterministic across reruns; the exact pin keeps the
    # fail-closed drift guard.
    default = execute_holds(expected_organic_deopts=340) + [
        expect("compile_requests", ">", 0),
...
def canary_execute_runner(
    *, threshold: int = 8, judges: list[Judge] | None = None
    ...
    # No organic deopts.  This pin held thirty-seven, all of them from the
    # child preamble's import machinery -- MR-08's exception opcode family
    # (EAFP raise paths deopt by design) and MR-09's guarded attribute
    # sites retiring as import-time types mutate.  MR-11 suppresses
    # scheduling inside import scopes, so none of that is compiled and
    # none of it deopts.  Zero is the sharper pin: it fails closed if
    # import-time compilation ever comes back.
...
def execute_auto_runner(
    *, threshold: int = 8, judges: list[Judge] | None = None
    ...
    organic scheduling installs machine code, one code object is compiled
    once however many function objects are created over it, fresh
    instances attach to the published artifact within the per-code budget
    (eight), and the mode reports itself as execute."""
    payload = EXECUTE_FRESH_DEF + CANARY_ATTEST + (
        ...
        "# Attachments are counted process-wide (the import machinery's own\n"
        "# closures attach too), so the factory's share is a delta.\n"
        "_attached = _cinderx._get_observe_stats()['fresh_attachments']\n"
        "_flags = []\n"
        "for n in range(20):\n"
        "    _f = factory(n)\n"
        "    for _ in range(12):\n"
        "        assert _f(2, 3, 1) == 3 * (2 + n), (n, _f(2, 3, 1))\n"
        "    _flags.append(cinderjit.is_jit_compiled(_f))\n"
        "    del _f\n"
        "# The compiled instance, eight attached ones, then interpreted.\n"
        "assert _flags[:9] == [True] * 9 and not any(_flags[9:]), _flags\n"
        "_after = _cinderx._get_trigger_stats()['compiled_function_creations']\n"
        "# adder once and the (hot) factory once: no compilation storm.\n"
        "assert _after - _made == 2, (_made, _after)\n"
        "_stats = _cinderx._get_observe_stats()\n"
        "assert _stats['fresh_attachments'] - _attached == 8, (\n"
        "    _attached, _stats['fresh_attachments'])\n"
        "_adder = [e for e in _stats['events']\n"
        "          if e['qualname'] == 'factory.<locals>.adder']\n"
        "assert len(_adder) == 1, _adder\n"
        "assert _adder[0]['result'] == 'installed', _adder\n"
    )
...
# Organic deopt pins for the execute-mode drivers, verified deterministic
# across reruns; the exact pins keep the fail-closed drift guard and are
# re-pinned only with an attribution.
#
# All of them are now zero, and the attribution is import/setup
# suppression.  Every count these pins used to hold came from the child
# preamble's import machinery: EAFP raise paths, attribute-cache receiver
# guards retiring as import-time types mutate, and -- 283 of the default
# row by itself -- typing._BaseGenericAlias.__setattr__, whose
# LOAD_GLOBAL_MODULE guard on `_is_dunder` retires on every call while
# the typing module's namespace is still being populated.  None of that
# is compiled any more: MR-11 suppresses scheduling inside import and
# setup scopes, so the machinery that produced these deopts now runs
# interpreted, which is the point of the suppression.
#
# Zero is still a fail-closed pin, and a sharper one than before: it now
# asserts that the suppression is working.  Import-time compilation
# returning for any reason makes these counts non-zero and fails the
# drivers.
