...
def _jit311_emit():
    ...
    # The logical lists answer "would a call enter machine code", and the
    # ownership contract makes that answer No the moment the evaluator is
    # gone -- so read them while it is still installed.  Sampled here at
    # Python atexit, before _cinderx module_free / jit::finalize();
    # finalize-after emptiness is a C-layer contract.  These are two single
    # reads; the multi-source snapshot stays behind the removal below.
    try:
...
# MR-05 lifecycle exercises.  Each one drives a death path that CPython
# 3.11 has no watcher for, and asserts through the notification counters
# that the JIT was actually told -- a leak shows up as a count that stops
# climbing while objects keep dying, which no behavioural assertion sees.
LIFECYCLE_PRELUDE = """\
import gc
import _cinderx
...
def _make_plain(name, body="    return x + 1\\n"):
    ns = {}
    exec("def %s(x):\\n%s" % (name, body), ns)
    # Take it out of its own globals.  exec() makes __globals__ be this
    # dict and the dict names the function, so leaving it there puts every
    # subject in a reference cycle and turns the exercise into a test of
    # when the collector runs rather than of when the JIT is told.
    return ns.pop(name)
...
# closure_churn: ten thousand short-lived closures and two thousand
# lambdas, every one of them pushed through the JIT's front door.  Closures
# carry cells, which the MR-04 execute surface refuses, so their arm proves
# the refusal path leaks nothing at churn rate.  The lambdas carry no cells
# and do compile -- all two thousand against the same code object, so each
# generation exercises the artifact succession a predecessor's death has to
# leave clean.
...
# weakref_reentry: query and uncompile the JIT from inside a callback that
# fires while the JIT is already unwinding the same function.  The reentry
# is mutating -- it force-uncompiles a live peer while the registries are
# mid-death for the subject -- and then walks the survivors.
...
# last_ref_during_action: a compiled function's last strong reference falls
# out of a dealloc batch in which the JIT's own death watch is a peer, in
# both interleavings (the user's weak reference armed before and after the
# JIT's), and the dropping callback itself performs a JIT operation on the
# victim while holding its final reference.  The strictly stronger form --
# the subject losing its last reference in the middle of the JIT operation
# itself -- cannot be built from Python, where the call machinery owns the
# argument for the duration; the native case
# JITLifecycle311Test.UncompileSurvivesLosingItsLastReferenceMidway drives
# that through the uncompile midpoint hook against the operation's pin.
LIFECYCLE_LAST_REF = """\
import weakref
