...
class A3CoreRunner(A3Runner):
    ...
    def run_hook(self) -> dict:
        ...
        result = {
            "result": "PASS" if statuses and not non_pass else "FAIL",
            "modules": len(statuses),
            "non_pass": non_pass,
            "poisoned": True,
            "errors": (
                []
                if statuses and not non_pass
                else [f"penetration arm non-pass modules: {non_pass or 'no results'}"]
            ),
        }
        (directory / "result.json").write_text(
    ...
    def run(self) -> str:  # type: ignore[override]
        ...
        infra = list(self.command_failures)
        infra.extend(
            f"{name} infrastructure failure"
            for name, case in cases.items()
            if case.get("result") == "INFRA_FAIL"
        )
        if infra:
            status = "INFRA_FAIL"
        elif all(case.get("result") == "PASS" for case in cases.values()) and (
            hook is None or hook.get("result") == "PASS"
        ):
            status = "PASS"
        else:
            status = "FAIL"
...
def main(argv: list[str] | None = None) -> int:
    ...
    return 0 if status == "PASS" else 2 if status == "FAIL" else 1
