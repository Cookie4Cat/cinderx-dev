"""A3 C1-C8 lifecycle churn discovery against the private numeric census."""
...
from ci_pipeline.jit311.a3_census import checkpoint, judge_plateau, snapshot
...
def _initialize():
    ...
    if not _cinderx.is_frame_evaluator_installed():
        raise RuntimeError("A3 requires the installed CPython 3.11 frame evaluator")
    if _cinderx._get_observe_stats().get("mode") != "execute":
        raise RuntimeError("A3 churn requires execute/canary mode")
