...
def test_stdlib_organic_deopt_count_drift_turns_red():
    ...
    # it at 333, the MR-11 base at 340); any drift off the pinned constant
    # must still turn red.
    errors = [
        error
        for judge in runners.stdlib_canary_runner().judges
        for error in judge({"organic_deopt_hits": 341})
    ]
    assert any("organic_deopt_hits == 340" in error for error in errors)
